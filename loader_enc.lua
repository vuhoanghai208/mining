...
-- Config dat BEN NGOAI: getgenv().OreSweepConfig = { ... } truoc khi chay loader nay.
...
for c in b64:gmatch("[^=]") do
    ...
    for p = 5, 0, -1 do bits[#bits + 1] = bit32.extract(i, p, 1) end
end
local out = {}
for i = 1, #bits - 7, 8 do
    local n = 0
    for p = 0, 7 do n = n * 2 + bits[i + p] end
    out[#out + 1] = string.char(bit32.bxor(n, k))
end
local url = table.concat(out)
getgenv().OreSweepLoader = 'loadstring(game:HttpGet("' .. url .. '"))()' -- script tu chay lai sau Server Hop
local ok, body = pcall(function() return game:HttpGet(url) end)
if not ok or type(body) ~= "string" then return warn("[OreSweep] khong tai duoc script: " .. tostring(body)) end
